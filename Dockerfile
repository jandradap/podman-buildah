...
RUN sed -i "s/# rootless_storage_path/rootless_storage_path/g" /etc/containers/storage.conf \
  && sed -i 's/overlay/vfs/1' /etc/containers/storage.conf \
  && sed -i 's/mount_program/#mount_program/1' /etc/containers/storage.conf
